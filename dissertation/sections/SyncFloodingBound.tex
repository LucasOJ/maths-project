...
\subsection{Review of Discrete-time Markov Chains}

In this section we give a brief review of Markov chains and associated results needed for Section \ref{subsection:MEDNBound}. We assume that the reader is familiar with finite Discrete-time Markov chains, so omit the proofs in this section. For a full treatment of the subject see \cite{grimmetBook}.
...
\subsection{Flooding on Markovian Evolving Dynamic Networks}\label{subsection:MEDNBound}
...
This indicates that our bound may not always be tight, which we will explore in Section \ref{subsect:edgeMarkovianFloodingBoundEvaluation}. % TODO: IS THIS TRUE?
...
Note that the lower bound on the spreading time is exactly the first term of the bound proved in Theorem \ref{theorem:edgeMarkovianFloodingBound}. Hence, when $\frac{\log n}{\log (n\hat{p})}$ is the dominant term, the bound is tight. Indeed, when $\hat{p} = \frac{c \log n}{n}$, % TODO: finish this 
...
\begin{figure}[h]
	\centering
	\includegraphics[width=1\textwidth]{./figures/flooding_simulation_1.5log.png}
	\caption{Results of Edge-Markovian Dynamic Network spreading time simulations when $\hat{p} = c\frac{(\log n)^\frac{3}{2}}{n}$}
	\label{fig:floodingSimResults1.5Log}
\end{figure}

Figure \ref{fig:floodingGnpSimResultsTight} displays the simulation results for rumour spread on the Stationary Edge-Markovian Network when $\hat{p} = \frac{c \log n}{n}$. 
Since the rumour spreads in discrete rounds, each of the simulation results in blue falls on an integer value. Each simulation was repeated 50 times for each network size, with the means displayed in red. In green, we have plotted the bound from Theorem \ref{theorem:edgeMarkovianFloodingBound}. As we expected, the mean simulation times closely follow the bound once an appropriate $c$ has been chosen. %TODO: Why expected, tidy this paragrpah

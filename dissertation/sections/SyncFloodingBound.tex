...
\begin{proof}
	...
	Since we have shown that the event $\bigcap_{t=0}^n A_t \implies B$, we have that $\comp{B} \implies \comp{ \bigcap_{t=0}^n A_t} = \bigcup_{t=0}^n \comp{A_t}$. To finish the proof, we show that the event $\comp{B}$ occurs with vanishing probability, thus $B$ occurs with high probability.
...
Now we apply Theorem \ref{theorem:DeterministicFloodingBound} to the Edge-Markovian Dynamic Network.

\begin{theorem}
	Let $\mathcal{M}(n, p, q)$ be a stationary Edge-Markovian Dynamic Network. If $\hat{p} = \frac{p}{p+q} \geq c \frac{\log n}{n}$ for a sufficiently large $c$ then with high probability, the spreading time of Algorithm \ref{SyncFloodingAlgorithm} in $\mathcal{M}(n, p, q)$ is at most
	$$
...
\begin{proof}
	...
	$$
	By substituting for $k_i$ in the summation, we have the that the order of the spreading time is bounded with high probability by
	$$
...
\begin{theorem}
	Let $\mathcal{M}(n, p, q)$ be a stationary Edge-Markovian Dynamic Network. If $\hat{p} = \frac{p}{p+q} \geq c \frac{\log n}{n}$ for a sufficiently large $c$ then with high probability, the spreading time of Algorithm \ref{SyncFloodingAlgorithm} in $\mathcal{M}(n, p, q)$ is at least 
	$$
...
\begin{proof}
	...
	$$
		...
		\frac{1}{n}
	$$
	Hence, with high probability, in the first $n$ time steps all nodes have degree less than $2n\hat{p}$.
	...
	$$
		\tau = \mathcal{O}\left(\frac{\log n}{\log(n \hat{p})}\right)
	$$
\end{proof}

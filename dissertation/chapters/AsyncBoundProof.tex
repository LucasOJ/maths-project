...
However, since the bound in Theorem \ref{theorem:AsyncUpperBound} holds w.h.p, we need a different metric that for which the majority of the spreading times are bounded by ... % TODO: Sort out what this means

To determine whether our bound is sufficient and tight, we investigate the growth rate of the orange line.

Since the asymptotic bound holds w.h.p, for a network with $n$ nodes, we expect that at most 1 in $n$ simulated spreading times exceed the bound.

For the network size of $n$ nodes, the corresponding value of the orange line represents the $100(1-\frac{1}{n})^\text{th}$ percentile of the simulation results, i.e. that 

We observe that the spreading times grow at like $f(n) = c \log \log (n)$ with an appropriately chosen scaling constant $c$ (plotted in green), so the $\mathcal{O}(\log n)$ bound holds. 

However, this analysis suggests that $\mathcal{O}(\log n)$ bound is not tight for $\mathcal{A}$.
...
We now consider another example of a dynamic network.

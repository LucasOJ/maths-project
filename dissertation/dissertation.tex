...
\begin{proof}
	...
	\noindent
	Since $|I_t| + |U_t| = n$ for all $t$, note that
	$$
		|I_{\gamma}| \geq |I_\tau| + \frac{m(\tau)}{2} 
		\iff
		n - |U_{\gamma}| \geq n - |U_\tau| + \frac{m(\tau)}{2} 
		\iff
		|U_\tau| - \frac{m(\tau)}{2} \geq |U_\gamma| 
	$$
	...
	Thus we have that
	\begin{align*}
		|U_\gamma| & \geq |U_\tau| - \frac{m(\tau)}{2} & \text{since }\gamma < \tau' \\
